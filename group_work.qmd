...
This analysis is based on data from **Inside Airbnb**, a widely used open dataset derived from publicly available information on the Airbnb platform. Although the data are not produced by an official government body, they are extensively used in academic research and policy analysis. The dataset is maintained by the Inside Airbnb Network, a mission-driven initiative that seeks to facilitate transparent public discussion about short-term rentals and their effects on local communities. Its open methodology and consistent use across peer-reviewed studies support its credibility as a data source.
...
# Automatically detecting missing libraries and installs them
...
# Checking and installing missing libraries
full_cmd = pip_cmd + pkgs

try:
   subprocess.check_call(full_cmd + ['--disable-pip-version-check'])
except subprocess.CalledProcessError:
   print("Standard install failed. Trying with --break-system-packages...")
   try:
      subprocess.check_call(full_cmd + ['--break-system-packages'])
   except subprocess.CalledProcessError as e:
      print(f"Critical Warning: Could not install packages. Error: {e}")
...
# Auto-downloading bio.bib and .csl -I asked AI how to do this
import os
# 1. download bio.bib 
bib_content = """
...
with open("bio.bib", "w", encoding="utf-8") as f:
    f.write(bib_content)
...
try:
    urllib.request.urlretrieve(csl_url, "harvard-cite-them-right.csl")
    print("CSL file downloaded successfully.")
except Exception as e:
    print(f"Failed to download CSL: {e}")
```
...
concentration_ratio = (target_n_total / n_misuses) * 100

print(f"Total number of properties affected by the opposition proposal in the target area: {total_target_affected}")
print(f"\nThese five districts have concentrated {concentration_ratio:.1f}% of the misused Airbnb listings in London.\n")
...
Based on our targeted strategy, **10,145** properties would be affected. Crucially, these five boroughs alone account for **81.8%** of all misuse properties in London. This concentration confirms the rationality of our targeted approach: **we can address the bulk of the "out of control" phenomenon by concentrating resources on just five boroughs**, maximizing impact while minimizing administrative costs.

## 7 What are the likely pros and cons of the opposition’s proposal? 

### 7.1 For the Mayor
The proposal offers the Mayor a chance to **demonstrate targeted, evidence-based leadership**. By focusing on the five "target boroughs“, the policy directly addresses areas containing 81.8% of London's misused listings, tackling the core problem without imposing excessive citywide restrictions.

Politically, the proposal is defensible as it targets a narrow and clearly defined group. With only 4.8% of hosts qualifying as professional landlords, over 95% remain unaffected, framing the intervention as proportionate.
